...
On larger instances, the LP crashes (using several different solvers). 
I had thought that this was because the numbers in the IP add up to $2^{n \choose k}$,
which grows very quickly.

However, several implementations are returning ``not feasible'', which
suggests that the constraints are incorrect. (Of these constraints, the
``step'' constraint seems the most dubious to me.)
