...
of the cliques in a graph. There is a trade-off: the more ``largest''
cliques we try to detect, the weaker the bound we get; the bound
...
the marginal distributions for each number of vertices, $E_[|\bigC_{V_v})]$,
and each layer, $E_[|\bigC_{L_l})]$. These are defined using constraints
...
\[
E[|\bigC_{V_n}|] \ge E[|\bigC_{V_{n-1}}|] + 1
\]

On its own, this constraint is not very useful; it only implies a lower
bound of $n-k$ gates, which is not very interesting.

\section{Results}

Having set this up, we minimize the number of gates in the highest layer,
$E[|\bigC_{L_L}|]$ for different
values of $L$, using up to 90 gates.
