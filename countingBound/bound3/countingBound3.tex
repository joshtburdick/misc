...
\subsection{Constraints using restrictions}

Restrictions (FIXME) are a standard tool in circuit complexity.
After setting an input of a circuit to a constant, some gates
may also have their inputs fixed to a constant.
Since we're using NAND gates, we know that feeding in
a 0 to some input will remove at least one gate. (Here, we
only consider feeding in 0's.)

We want to show that sets of cliques with more vertices
require more gates to detect, on average. We can visualize this
as a tree; we want to show that each level of the tree is higher.

We need to be slightly careful here, though; it's perfectly possible
for the averages at a given level in such a tree to not be increasing.

(FIXME add counterexample)

However, if we make sure that the degree of nodes at each level
is the same, then the average at each level will be increasing.
...
\[
E[|\bigC_{V_v}|] \ge E[|\bigC_{V_{v-1}}|] + 1
\]
...
It may look incorrect, or at least surprising, that the LP is conjecturing
that finding small sets of cliques is in fact harder. One explanation
would be that as the LP pushes the average for the functions with 
many cliques down, it ``pushes out'' the functions with few cliques, making it appear that they are harder to compute.
(Indeed, it seems possible that adding the naive upper bound, for the lower layer,
would improve the bounds for the top layer.)
...
on the highest layer is large-ish, $CLIQUE$ might still be easy to compute.
