...
We could imagine a miles-long linear warehouse of chips with the minimal
circuit for each problem, sorted in terms of number of gates.
Or, with apologies to languages which aren't written left-to-write,
...
This means that
$C$ is, on average, $a/2$ functions ``to the right of'' $B$,
implying at least that large a difference in rank.

\end{proof}

In terms of the previous warehouse analogy, we can imaging that,
...
Step case: Consider any level $l$.

Pick a random set of cliques from that level (which can be thought of
as a hypergraph with $l$ hyperedges). Feed in a 0 to a randomly-chosen
edge. We know that at least one gate will be ``zonked'' by this.
We can also approximate how many cliques will be ``disabled'' by this.

??? how accurate is this approximation?

Note that at higher levels, the number of cliques disabled will be
fairly large. However, no more than ${n-2 \choose k-2}$ cliques can
be disabled, which is a fair amount less than $n \choose k$.
