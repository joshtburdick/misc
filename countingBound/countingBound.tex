...
and numerically evaluate this bound for small graphs. We then
discuss prospects for finding a bound in closed form.
\end{abstract}

I am writing this up on the assumption that the modification
to Shannon's counting argument (section \ref{countingBound})
is not new. I haven't seen the rest before, but obviously it makes some
strong claims.
...
Choose some coverage strategy, and let:

\begin{itemize}

\item $A_{ij}$ = the average number of $i$-vertex cliques in a $j$-vertex graph

\item $b_i$ = the Shannon bound on the number of
gates needed to find $k$-cliques on $n$ vertices.

\item $x_j$ = the number of gates needed to find $k$-cliques on $j$ vertices.
...
This gives a lower bound on $x$ (the number of gates needed to find
$k$-cliques in $j$-vertex graphs). The bound depends
crucially on how efficiently
we can cover the hyperedges with hypercliques -- in order for $x$ to
be large, we need entries of $A$ to be as small as we can manage.
...
try to apply the simplest strategy in section FIXME.
...
This ``RISC-like'' \cite{hennessy2011computer} strategy
...
We now try to see what, if anything, this bound implies. Note again that
...
then using Julia's arbitrary-precision arithmetic.
...
problem (\cite{ross2006first}, p. 344).
...
When I tried maximizing for larger problems, I started using up my laptop's
memory (all 8 Gb of it). Also, the bound was fairly slow. I assume that the
...
based on an approximation based on the fact that
...
to lower bound strategies. He would also like to thank
... for maintaining interesting relevant blogs.
