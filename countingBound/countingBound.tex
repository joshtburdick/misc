...
\begin{document}
\title{A lower bound for clique detection, by combining
...
$n$-vertex graph, on average, requires $\Omega(n^{k/2})$ NAND gates.
This doesn't directly bound the complexity of clique detection.
However, we can view a random subset of cliques as a randomly-chosen
$k$-regular on $n$ vertices.
If we can cover such a hypergraph with large enough cliques,
then if we could detect cliques with few enough gates, we would contradict
the average-case counting bound described above.
This provides a strategy for lower-bounding clique detection.
We numerically evaluate this bound for small graphs, and discuss prospects
for writing it in closed form.
\end{abstract}
...
strong claims.
I'm hopeful that it is correct and/or, or that it has useful bits,
...
then to detect all the triangles, we will definitely need at least one
gate per triangle.
...
function-counting bound gives a lower-bound the total number of gates used (as from the
...
Also, probabilistic graph theory has often been used in
...
This lower-bound strategy also seems relevant to quantum computing,
as it makes few restrictions on the sort of gates used.
