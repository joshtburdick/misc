...
To clarify this, we
plot an example set of cliques for each point on this rectangle
...
Note that the above bound didn't use the structure of the lattice $Z$ 
(figure \ref{fig:Z}) at all.
Since we're using NAND gates, every edge along a path from $CLIQUE$ to $\emptyset$
implies a slightly smaller circuit.

We may also try counting gates instead of rank, and using the upper bound from section
\ref{gateUpperBound}. $CLIQUE$ is only directly
