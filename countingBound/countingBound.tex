...
FIXME the subscripts are all off here...

\begin{itemize}

\item $A_{rn}$ = the average number of $r$-vertex hypercliques (``$K^r_k$''s)
in an $n$-vertex graph

\item $X_{r}$ = the number of gates needed to find $k$-cliques on $n$ vertices.

\item $B_{rk}$ = the Shannon bound on the number of
