...
class LpVertexZeroing:
    ...
    def __init__(self, n, k, max_gates):
        ...
        for v in range(k, n + 1):
            for c in range(comb(v, k) + 1):
                ...
                self.num_gates_dist_vars += [
                    ("g", v, c, g) for g in range(1, max_gates + 1)
                ]
    ...
    def add_averaging_constraints(self):
        ...
        for v in range(self.k, self.n + 1):
            ...
            for c in range(num_possible_cliques + 1):
                ...
                A = [(("g", v, c, i), i) for i in range(1, self.max_gates + 1)]
                self.lp.add_constraint(A + [(("u", v, c), -num_functions)], "=", 0)
    ...
    def add_counting_bound(self):
        ...
        """
        num_functions = self.basis.num_functions(
            self.num_possible_cliques, self.max_gates
        )
        ...
        for v in range(self.k + 1, self.n + 1):
            ...
            for C_size in range(1, comb(v, self.k) + 1):
                ...
                p_hit = np.array(
                    [
                        hyperg_frac(
                            comb(v, self.k), C_size, num_cliques_hitting_vertex, h
                        )
                        for h in B_size
                    ]
                )
                ...
                print(
                    f"v={v}, C_size={C_size}, B_size={B_size}, p_hit={p_hit}, upper_bound={upper_bound}"
                )
                if use_upper_bound:
    ...
    def get_all_bounds(self, band_width):
        ...
        num_functions = np.array(
            [comb(self.num_possible_cliques, self.k - i) for i in range(band_width)],
            dtype=fractions.Fraction,
        )
        p_functions = num_functions / (num_functions.sum() + 1)
        objective = {
            ("u", self.n, self.num_possible_cliques - i): p_functions[i]
        ...
        return pandas.DataFrame(
            {"Num. vertices": self.n, "Num. cliques": n_cliques, "Min. gates": bounds}
        )
...
def get_bounds(
    ...
    constraints_label,
    use_counting_bound,
    use_zeroing_lower_bound,
    use_zeroing_upper_bound,
):
    ...
    sys.stderr.write(
        f"[bounding with n={n}, k={k}, max_gates={max_gates}, label={constraints_label}]\n"
    )
    ...
    bound.add_vertex_zeroing_constraints(
        use_zeroing_lower_bound, use_zeroing_upper_bound
    )
...
def parse_args():
    ...
    parser.add_argument(
        "band_width",
        type=int,
        help="width of highest possible number of cliques to minimize",
    )
    ...
    bounds = pandas.concat(
        [
            ...
            get_bounds(
                ...
                args.max_gates,
                args.band_width,
                "Counting, zeroing lower and upper",
                True,
                True,
                True,
            ),
