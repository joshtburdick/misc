...
class LpParityLayers:
    ...
    def __init__(self, n, k, num_layers):
        ...
        # Make the layers. We use half-open intervals (to work with
        # Python's range() function). Note that we require these to be
    ...
    def make_layers(self, num_layers):
        ...
        # for now, assume number of layers is odd
        if num_layers % 2 == 0:
            raise ValueError("Number of layers must be odd.")
        def make_layer(vertices):
            """Makes one layer of the variables, for some number of vertices."""
            N = comb(self.n, self.k) + 1
            height = (N+1 / num_layers)
            a = [height * i for i in range(num_layers//2)]
            low = [(a, a+height) for a in a]
            high = [(N-a+1, N-b+1) for a,b in reversed(low)]
            middle= (low[-1][1], high[0][0])
